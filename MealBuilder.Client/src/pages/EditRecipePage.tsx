...
import type { Recipe, RecipeInput } from '../api/recipeApi'
...
export default function EditRecipePage() {
  ...
  const ingredientRefreshErrors = useIngredientRefresh({
    enabled: !isLoading && !isSubmitting && recipe !== null,
  ...
  async function handleSubmit(input: RecipeInput) {
    ...
    try {
      const updatedRecipe = await recipeApi.update(recipe.id, input)

      navigate(`/library/recipes/${updatedRecipe.id}`, { replace: true })
    } catch (error) {
      setErrors(getApiErrorMessages(error, 'Unable to update the Recipe.'))
    } finally {
  ...
  if (!recipe) {
    return (
      <section className="recipe-form-page">
        ...
        <Link className="recipe-form__cancel" to="/library/recipes">
          Back to Recipes
        </Link>
      </section>
    )
  ...
  return (
    <section className="recipe-form-page">
      <header className="recipe-form-page__header">
        ...
        <p>Update {recipe.name}, its Ingredients, and Cooking Steps.</p>
      </header>
